...
##############
dfbezt<-df[which(df$Mjerenje.I.T.F.!= 'tranzitivno'),]
itm<-dfbezt %>% group_by(Mjerenje.I.T.F.,SportGrupa) %>% summarize(srITM = mean(ITM))
gmeanITM <- ggplot(data = itm) + geom_bar(aes(SportGrupa,srITM,fill=SportGrupa),stat='identity') +
          facet_grid(.~Mjerenje.I.T.F.)
mdspair<-pairwise.t.test(df$MDS,df$SportGrupa)

###

modelMTR <- aov(MTR ~ (SportGrupa*Tezinska.grupa*Mjerenje.I.T.F.) +
                  Error(R.br./(Mjerenje.I.T.F.)),data=dfbezt)
modelMDS <- aov(MDS ~ (SportGrupa*Tezinska.grupa*Mjerenje.I.T.F.) +
                  Error(R.br./(Mjerenje.I.T.F.)),data=dfbezt)
modelMPN <- aov(MPN ~ (SportGrupa*Tezinska.grupa*Mjerenje.I.T.F.) +
                  Error(R.br./(Mjerenje.I.T.F.)),data=dfbezt)
...
laki <-dfbezt[which(dfbezt$Tezinska.grupa==1),]
teski <-dfbezt[which(dfbezt$Tezinska.grupa==3),]
###

laovMPN<-aov(MPN~SportGrupa * Mjerenje.I.T.F.,data=laki)
modelITM <- aov(ITM ~ SportGrupa * Mjerenje.I.T.F.*Tezinska.grupa +
                  Error(R.br./(Mjerenje.I.T.F.)),data=dfbezt)

###
cor(dfbezt$MTR, dfbezt$ITM,method='pearson')
cor(dfbezt$MDS, dfbezt$ITM,method='pearson')
cor(dfbezt$MPN, dfbezt$ITM,method='pearson')
cor(dfbezt$MPT, dfbezt$ITM,method='pearson')
cor(dfbezt$MPR, dfbezt$ITM,method='pearson')
cor(dfbezt$MIV, dfbezt$ITM,method='pearson')
cor(dfbezt$F6, dfbezt$ITM,method='pearson')
